...
function RecipeData_checkRecipe(fruit, liquid, ice) {
  return RecipeData_RECIPES.find(r =>
    r.fruit === fruit && r.liquid === liquid && r.ice === ice
  ) || null;
}
...
function RecipeData_populateRecipeBook() {
  ...
  RecipeData_RECIPES.forEach(r => {
    ...
    card.innerHTML = `<h4>${r.emoji} ${r.name}</h4><p>${RecipeData_FRUIT_EMOJIS[r.fruit]} ${r.fruit.charAt(0).toUpperCase() + r.fruit.slice(1)} + ${liquidEmoji} ${r.liquid.charAt(0).toUpperCase() + r.liquid.slice(1)}${iceText} → $${r.price}</p>`;
